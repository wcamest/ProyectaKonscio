...
/**
 * Función para mover un nodo hacia abajo dentro de un documento visual en un editor.
 ...
 * @returns El nuevo estado del editor visual después de mover el nodo hacia abajo.
 */
export default function MoveNodeDown(
  ...
  // Identifica el ID del nodo que se va a mover hacia abajo.
  ...
  // Si el nodo que se va a mover ya está en la parte inferior, devuelve el estado sin cambios.
  if (nodeIndex === parentNodeToMove.nodes.length - 1) return state;

  // Crea una copia de la lista de nodos del nodo padre y reorganiza los nodos para mover el nodo hacia abajo.
